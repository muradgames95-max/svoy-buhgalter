  ...
  return (
    <div className="space-y-4 pt-2">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        {/* Bulk action for pending filter */}
        {statusFilter === 'pending' && filtered.length > 0 && (
          <div className="px-5 py-2.5 bg-amber-50 border-b border-amber-100 flex items-center justify-between">
            <span className="text-xs text-amber-700 font-medium">{filtered.length} ожидает оплаты</span>
            <button
              onClick={() => updateIncomes(incomes.map((i) => filtered.some((f) => f.id === i.id) ? { ...i, status: 'paid' as const } : i))}
